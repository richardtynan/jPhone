...
DEFUN(select_cell, select_cell_cmd, "select cell MS ARFCN",
	"select tower")
{
struct osmocom_ms *ms = get_ms(argv[0], vty);
struct gsm322_cellsel *cs = &ms->cellsel;
cs->proposed_arfcn = atoi(argv[1]);
vty_notify(ms, "selecting tower %d\n", cs->proposed_arfcn);
return CMD_SUCCESS;
}
...
int ms_vty_init(void)
{
	...
	install_element_ve(&select_cell_cmd);
