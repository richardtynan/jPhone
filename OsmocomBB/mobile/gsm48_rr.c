...
static int gsm48_rr_tx_cip_mode_cpl(struct osmocom_ms *ms, uint8_t cr)
{
	...
	if (cr) {
		...
		vty_notify(ms, " %d %d generating mid from imsi/imeisv - gsm48_rr_tx_cip_mode_cpl", cs->sel_arfcn, cs->sel_id);
		/* alter MI type */
...
static int gsm48_rr_rx_sysinfo1(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	//vty_notify(ms, "SYSINFO:01:GSM48RR ARFCN=%d BSIC=%d MCC=%d MNC=%d LAC=0x%04x Cell ID=0x%04x\n", 
	//	s->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);

	gsm48_decode_sysinfo1(s, si, msgb_l3len(msg));

	vty_notify(ms, "SYSINFO:01:GSM48RR ARFCN=%d BSIC=%d MCC=%x MNC=%x LAC=0x%04x Cell ID=0x%04x\n", 
		cs->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
...
static int gsm48_rr_rx_sysinfo2(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	//vty_notify(ms, "SYSINFO:02:GSM48RR ARFCN=%d BSIC=%d MCC=%d MNC=%d LAC=0x%04x Cell ID=0x%04x\n", 
	//	s->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);	
	gsm48_decode_sysinfo2(s, si, msgb_l3len(msg));
	vty_notify(ms, "SYSINFO:02:GSM48RR ARFCN=%d BSIC=%d MCC=%x MNC=%x LAC=0x%04x Cell ID=0x%04x\n", 
		cs->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
...
static int gsm48_rr_rx_sysinfo2bis(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	//vty_notify(ms, "SYSINFO:2b:GSM48RR ARFCN=%d BSIC=%d MCC=%d MNC=%d LAC=0x%04x Cell ID=0x%04x\n", 
	//	s->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
	gsm48_decode_sysinfo2bis(s, si, msgb_l3len(msg));
	vty_notify(ms, "SYSINFO:2b:GSM48RR ARFCN=%d BSIC=%d MCC=%x MNC=%x LAC=0x%04x Cell ID=0x%04x\n", 
		cs->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
...
static int gsm48_rr_rx_sysinfo2ter(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	if (!memcmp(si, s->si2t_msg, MIN(msgb_l3len(msg), sizeof(s->si2t_msg))))
		return 0;
	//vty_notify(ms, "SYSINFO:2t:GSM48RR ARFCN=%d BSIC=%d MCC=%d MNC=%d LAC=0x%04x Cell ID=0x%04x\n", 
	//	s->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
	gsm48_decode_sysinfo2ter(s, si, msgb_l3len(msg));
	vty_notify(ms, "SYSINFO:2t:GSM48RR ARFCN=%d BSIC=%d MCC=%x MNC=%x LAC=0x%04x Cell ID=0x%04x\n", 
		cs->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
...
static int gsm48_rr_rx_sysinfo3(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	//vty_notify(ms, "SYSINFO:03:GSM48RR ARFCN=%d BSIC=%d MCC=%d MNC=%d LAC=0x%04x Cell ID=0x%04x\n", 
	//	s->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
	gsm48_decode_sysinfo3(s, si, msgb_l3len(msg));
	vty_notify(ms, "SYSINFO:03:GSM48RR ARFCN=%d BSIC=%d MCC=%x MNC=%x LAC=0x%04x Cell ID=0x%04x\n", 
		cs->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
...
static int gsm48_rr_rx_sysinfo4(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	if (!memcmp(si, s->si4_msg, MIN(msgb_l3len(msg), sizeof(s->si4_msg))))
		return 0;
	//vty_notify(ms, "SYSINFO:04:GSM48RR ARFCN=%d BSIC=%d MCC=%d MNC=%d LAC=0x%04x Cell ID=0x%04x\n", 
	//	s->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
	gsm48_decode_sysinfo4(s, si, msgb_l3len(msg));
	vty_notify(ms, "SYSINFO:04:GSM48RR ARFCN=%d BSIC=%d MCC=%x MNC=%x LAC=0x%04x Cell ID=0x%04x\n", 
		cs->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
...
static int gsm48_rr_rx_sysinfo5(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	if (!memcmp(si, s->si5_msg, MIN(msgb_l3len(msg), sizeof(s->si5_msg))))
		return 0;
	//vty_notify(ms, "SYSINFO:05:GSM48RR ARFCN=%d BSIC=%d MCC=%d MNC=%d LAC=0x%04x Cell ID=0x%04x\n", 
	//	s->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
	gsm48_decode_sysinfo5(s, si, msgb_l3len(msg));
	vty_notify(ms, "SYSINFO:05:GSM48RR ARFCN=%d BSIC=%d MCC=%x MNC=%x LAC=0x%04x Cell ID=0x%04x\n", 
		cs->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
...
static int gsm48_rr_rx_sysinfo5bis(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	if (!memcmp(si, s->si5b_msg, MIN(msgb_l3len(msg),
			sizeof(s->si5b_msg))))
		return 0;
	//vty_notify(ms, "SYSINFO:5b:GSM48RR ARFCN=%d BSIC=%d MCC=%d MNC=%d LAC=0x%04x Cell ID=0x%04x\n", 
	//	s->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
	gsm48_decode_sysinfo5bis(s, si, msgb_l3len(msg));
	vty_notify(ms, "SYSINFO:5b:GSM48RR ARFCN=%d BSIC=%d MCC=%x MNC=%x LAC=0x%04x Cell ID=0x%04x\n", 
		cs->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
...
static int gsm48_rr_rx_sysinfo5ter(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	if (!memcmp(si, s->si5t_msg, MIN(msgb_l3len(msg),
			sizeof(s->si5t_msg))))
		return 0;
	//vty_notify(ms, "SYSINFO:5t:GSM48RR ARFCN=%d BSIC=%d MCC=%d MNC=%d LAC=0x%04x Cell ID=0x%04x\n", 
	//	s->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
	gsm48_decode_sysinfo5ter(s, si, msgb_l3len(msg));
	vty_notify(ms, "SYSINFO:5t:GSM48RR ARFCN=%d BSIC=%d MCC=%x MNC=%x LAC=0x%04x Cell ID=0x%04x\n", 
		cs->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
...
static int gsm48_rr_rx_sysinfo6(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	if (!memcmp(si, s->si6_msg, MIN(msgb_l3len(msg), sizeof(s->si6_msg))))
		return 0;
	//vty_notify(ms, "SYSINFO:06:GSM48RR ARFCN=%d BSIC=%d MCC=%d MNC=%d LAC=0x%04x Cell ID=0x%04x\n", 
	//	s->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
	gsm48_decode_sysinfo6(s, si, msgb_l3len(msg));
	vty_notify(ms, "SYSINFO:06:GSM48RR ARFCN=%d BSIC=%d MCC=%x MNC=%x LAC=0x%04x Cell ID=0x%04x\n", 
		cs->arfcn, s->bsic, s->mcc, s->mnc, s->lac, s->cell_id);
...
static uint8_t gsm_match_mi(struct osmocom_ms *ms, uint8_t *mi)
{
		...
		if (ms->subscr.tmsi == ntohl(tmsi)
		 ...
		 && ms->subscr.lac == cs->sel_lac) {
			LOGP(DPAG, LOGL_INFO, " TMSI %08x matches\n",
				ntohl(tmsi));
			vty_notify(ms, "TMSI matches - gsm_match_mi\n");
			return mi_type;
		} else
	...
	case GSM_MI_TYPE_IMSI:
		...
		if (!strcmp(imsi, ms->subscr.imsi)) {
			...
			vty_notify(ms, "IMSI matches - gsm_match_mi\n");
			return mi_type;
		} else
		...
		if (ms->subscr.tmsi != 0xffffffff
		 ...
		 && rr->paging_mi_type == GSM_MI_TYPE_TMSI) {
			...
			LOGP(DRR, LOGL_INFO, "sending paging response with "
				"TMSI\n");
			vty_notify(ms, "sending paging response with TMSI\n");
		} else if (subscr->imsi[0]) {
			gsm48_generate_mid_from_imsi(mi, subscr->imsi);
			LOGP(DRR, LOGL_INFO, "sending paging response with "
				"IMSI\n");
			vty_notify(ms, "sending paging response with IMSI - gsm48_rr_dl_est\n");
		} else {
			mi[1] = 1;
			mi[2] = 0xf0 | GSM_MI_TYPE_NONE;
			LOGP(DRR, LOGL_INFO, "sending paging response without "
				"TMSI/IMSI\n");
			vty_notify(ms, "sending paging response without TMSI/IMSI - gsm48_rr_dl_est\n");
		}
