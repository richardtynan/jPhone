...
int gsm322_plmn_sendmsg(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	//vty_notify(ms, "Queue PLMN selection message\n");
...
int gsm322_cs_sendmsg(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	//vty_notify(ms, "Queue message to select cell %x\n", cs->sel_id);
	...
	uint16_t mnc, int any)
{
	...
	vty_notify(ms, "proposed select - %d\n", cs->proposed_arfcn);
	if (cs->proposed_arfcn != 0) {
		//vty_notify(ms, "proposed select - %d\n", cs->proposed_arfcn);
		start = end = cs->proposed_arfcn;
		cs->proposed_arfcn = 0;	
		//vty_notify(ms, "proposed select - %d %d\n", cs->proposed_arfcn, end);
	}
	for (i = start; i <= end; i++) {
		...
		if (found < 0 || cs->list[i].rxlev > power) {
			...
			found = i;
			//vty_notify(ms, "Selected cell %x as best.\n", s->cell_id);
		}
	}
	...
	if (found >= 0) {
		...
		vty_notify(ms, "Cell ARFCN %s selected.\n",
			gsm_print_arfcn(index2arfcn(found)));
	
} else {vty_notify(ms, "No suitable cell found\n");}
	return found;
}
...
static int gsm322_cs_store(struct osmocom_ms *ms)
{
	...
	//struct gsm48_sysinfo *csi = cs->list[cs->arfci].sysinfo;
	//vty_notify(ms, "SYSINFO:GSM322 ARFCN=%d BSIC=%d MCC=%d MNC=%d LAC=0x%04x Cell ID=0x%04x\n", 
	//	csi->arfcn, csi->bsic, csi->mcc, csi->mnc, csi->lac, csi->cell_id);
	/* store sysinfo */
	cs->list[cs->arfci].flags |= GSM322_CS_FLAG_SYSINFO;
...
indicate_plmn_avail:
	...
	cs->sel_lac = cs->si->lac;
	cs->sel_id = cs->si->cell_id;
	vty_notify(ms, "CELLSELECTED:GSM322 ARFCN=%d BSIC=%d MCC=%x MNC=%x LAC=0x%04x Cell ID=0x%04x\n", 
		cs->sel_arfcn, cs->sel_si.bsic, cs->sel_mcc, cs->sel_mnc, cs->sel_lac, cs->sel_id);
	
	if (ms->rrlayer.monitor) {
		vty_notify(ms, "MON: %s cell selected ARFCN=%s MCC=%s MNC=%s "
	...
	if ((cs->state == GSM322_C3_CAMPED_NORMALLY
	 ...
	 && (cs->neighbour)) {
		if (s->si3 || s->si4) {
			...
			int temp = gsm322_nb_read(cs, 1);
			//gsm322_vty_dump_cs_list(ms, cs);
			return temp;
		}
		//gsm322_vty_dump_cs_list(ms, cs);
		return 0;
...
printf("%d time to sync again: %u\n", nb->arfcn, now + GSM58_READ_AGAIN - nb->when);
	...
	//vty_notify(ms, "proposed trigger - %d\n", cs->proposed_arfcn);
...
int gsm322_vty_dump_cs_list(struct osmocom_ms *ms, struct gsm322_cellsel *cs)
{
	int i;
	struct gsm48_sysinfo *s;

	vty_notify(ms, "ARFCN  |MCC    |MNC    |LAC    |cell ID\n");
	for (i = 0; i <= 1023+299; i++) {
		s = cs->list[i].sysinfo;
		if (!s)
			continue;
		...
		if (i >= 1024)
			vty_notify(ms, "%4dPCS|", i-1024+512);
		else if (i >= 512 && i <= 885)
			vty_notify(ms, "%4dDCS|", i);
		else
			vty_notify(ms, "%4d   |", i);

		if (s->mcc) {
			vty_notify(ms, "%s    |%s%s    |0x%04x |0x%04x \n", gsm_print_mcc(s->mcc),
				gsm_print_mnc(s->mnc),
				((s->mnc & 0x00f) == 0x00f) ? " ":"", s->lac, s->cell_id);
		} else
			vty_notify(ms, "n/a    |n/a    |n/a    |n/a    \n");
	}
	vty_notify(ms, "\n");	
	return 0;
}
