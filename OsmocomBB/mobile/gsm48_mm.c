	...
	uint8_t mi_type)
{
	...
	case GSM_MI_TYPE_TMSI:
		...
		vty_notify(ms, "encoding mi using TMSI\n");		
		break;
	case GSM_MI_TYPE_IMSI:
		gsm48_generate_mid_from_imsi(buf, subscr->imsi);
		vty_notify(ms, "encoding mi using IMSI\n");
		break;
	case GSM_MI_TYPE_IMEI:
		gsm48_generate_mid_from_imsi(buf, set->imei);
		vty_notify(ms, "encoding mi using IMEI/IMSI\n");
		break;
	case GSM_MI_TYPE_IMEISV:
		gsm48_generate_mid_from_imsi(buf, set->imeisv);
		vty_notify(ms, "encoding mi using IMEISV/IMSI\n");
		break;
	...
	uint8_t sapi, uint8_t cause)
{
	...
	rrh->sapi = sapi;
	rrh->cause = cause;
	//vty_notify(ms, "Message sending to RR\n");
	/* send message to RR */
...
static int gsm48_mm_cell_selected(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	//vty_notify(ms, "Attempting to select cell %x\n", cs->sel_id);
	...
	if (subscr->ustate == GSM_SIM_U1_UPDATED
	 ...
	 && !mm->lupd_periodic) {
		if (subscr->imsi_attached) {
			...
			new_mm_state(mm, GSM48_MM_ST_MM_IDLE,
				GSM48_MM_SST_NORMAL_SERVICE);
			//vty_notify(ms, "Already attached to %x\n", cs->sel_id);
		...
		if (!s->att_allowed) {
			...
			new_mm_state(mm, GSM48_MM_ST_MM_IDLE,
				GSM48_MM_SST_NORMAL_SERVICE);
			vty_notify(ms, "Attachment not required to %x\n", cs->sel_id);
			/* send message to PLMN search process */
			nmsg = gsm322_msgb_alloc(GSM322_EVENT_REG_SUCCESS);
			if (!nmsg)
				return -ENOMEM;
...
static int gsm48_mm_tx_mm_status(struct osmocom_ms *ms, uint8_t cause)
{
	...
	*reject_cause = cause;
	...
	//vty_notify(ms, "Message sending mm tx mm status\n");
...
static int gsm48_mm_tx_tmsi_reall_cpl(struct osmocom_ms *ms)
{
	...
	/* push RR header and send down */
	//vty_notify(ms, "Message sending mm tx tmsi reall cpl\n");
	return gsm48_mm_to_rr(ms, nmsg, GSM48_RR_DATA_REQ, 0, 0);
}
...
static int gsm48_mm_tx_auth_rsp(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	/* push RR header and send down */
	//vty_notify(ms, "Message sending mm tx auth resp\n");
	return gsm48_mm_to_rr(ms, nmsg, GSM48_RR_DATA_REQ, 0, 0);
}
...
static int gsm48_mm_rx_id_req(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	vty_notify(ms, "IDENTITY REQUEST (mi_type %d)\n", mi_type);
...
static int gsm48_mm_tx_id_rsp(struct osmocom_ms *ms, uint8_t mi_type)
{
	...
	/* push RR header and send down */
	//vty_notify(ms, "Message sending mm tx id rsp\n");	
	return gsm48_mm_to_rr(ms, nmsg, GSM48_RR_DATA_REQ, 0, 0);
}
...
static int gsm48_mm_tx_imsi_detach(struct osmocom_ms *ms, int rr_prim)
{
	...
	if (subscr->tmsi != 0xffffffff) { /* have TMSI ? */
		...
		vty_notify(ms, " using TMSI %d for imsi detach\n", subscr->tmsi);
	} else {
		gsm48_encode_mi(buf, nmsg, ms, GSM_MI_TYPE_IMSI);
		LOGP(DMM, LOGL_INFO, " using IMSI %s\n", subscr->imsi);
		vty_notify(ms, " using IMSI %s for imsi detach\n", subscr->imsi);
	}

	/* push RR header and send down */
	mm->est_cause = RR_EST_CAUSE_OTHER_SDCCH;
	//vty_notify(ms, "Message sending mm tx imsi detach\n");
...
static int gsm48_mm_tx_loc_upd_req(struct osmocom_ms *ms)
{
	...
	if (subscr->tmsi != 0xffffffff) { /* have TMSI ? */
		...
		LOGP(DMM, LOGL_INFO, " using TMSI 0x%08x\n", subscr->tmsi);
		vty_notify(ms, " using TMSI %d for tx loc upd req to %x\n", subscr->tmsi, cs->sel_id);
	} else {
		gsm48_encode_mi(buf, NULL, ms, GSM_MI_TYPE_IMSI);
		LOGP(DMM, LOGL_INFO, " using IMSI %s\n", subscr->imsi);
		vty_notify(ms, " using IMSI %s for tx loc upd req to %x\n", subscr->imsi, cs->sel_id);
	}
	...
	//vty_notify(ms, "Message sending mm tx loc upd req\n");
	//vty_notify(ms, "Selected cell %x\n", cs->sel_id);
	...
	uint8_t cm_serv)
{
	...
	if (subscr->tmsi != 0xffffffff) { /* have TMSI ? */
		...
		vty_notify(ms, " using TMSI %d for cm serv req\n", subscr->tmsi);
	} else {
		gsm48_encode_mi(buf, NULL, ms, GSM_MI_TYPE_IMSI);
		LOGP(DMM, LOGL_INFO, "-> Using IMSI %s\n",
			subscr->imsi);
		vty_notify(ms, " using IMSI %s for cm serv req\n", subscr->imsi);
	}
	...
	/* push RR header and send down */
	//vty_notify(ms, "Message sending mm tx cm serv req\n");	
	return gsm48_mm_to_rr(ms, nmsg, rr_prim, 0, mm->est_cause);
}
...
static int gsm48_mm_tx_cm_service_abort(struct osmocom_ms *ms)
{
	...
	/* push RR header and send down */
	//vty_notify(ms, "Message sending mm tx service abort\n");
	return gsm48_mm_to_rr(ms, nmsg, GSM48_RR_DATA_REQ, 0, 0);
}
...
static int gsm48_mm_conn_go_dedic(struct osmocom_ms *ms)
{
	...
	case GSM48_PDISC_SMS:
		if (!mm->sapi3_link) {
			...
			if (!nmsg)
				return -ENOMEM;
			//vty_notify(ms, "Message sending mm con go dedic\n");			
...
static int gsm48_mm_data(struct osmocom_ms *ms, struct msgb *msg)
{
	...
	/* push RR header and send down */
	//vty_notify(ms, "Message sending mm data\n");
